...
function approveContribution(contribution_id) {
	fetch("/admin/approve-contribution", {
			method: "POST",
			body: JSON.stringify({
		...
		.then((response) => {
			if (response.status == 200) {
				response.json().then((data) => {
					if (data["status"] == "success") {
						alert(data["message"]);
						let contribution = document.getElementById(contribution_id);
						contribution.parentNode.removeChild(contribution);
					}
				});
			} else {
				alert("An error occurred while approving the contribution.");
			}
		});
}

function rejectContribution(contribution_id) {
	var result = window.confirm("Are you sure you want to reject this contribution?");

	if (result) {

		fetch("/admin/reject-contribution", {
				method: "POST",
				body: JSON.stringify({
					contribution_id: contribution_id,
				}),
